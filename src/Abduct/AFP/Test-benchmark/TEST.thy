...
lemma test :
  assumes p_k : "P k" and j_a : "k = a" and j_k : "a = j" 
  shows "P j"
proof - 
  (* This apply step runs the abduction, prints the result, 
     but keeps the goal state unchanged (like a diagnostic command) *)  
  (* The goal state is exactly the same as before the apply. 
     You can now copy-paste the result from the Output panel. *)
  try_abduct
