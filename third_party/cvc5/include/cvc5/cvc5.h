...
class CVC5_EXPORT Solver
{
 ...
 public:
  ...
  /**
   * Get an abduct using a subset of assertions for grammar generation.
   *
   * This is a variant of getAbduct() that allows specifying which assertions
   * should be used for grammar generation (symbol extraction and constant
   * collection). All current assertions are still used for the logical
   * abduction constraint.
   *
   * @warning This function is experimental and may change in future versions.
   *
   * @param conj The conjecture term.
   * @param grammarAssertions The subset of current assertions to use for
   *        grammar generation.
   * @return The abduct term, or null if none found.
   */
  Term getAbduct(const Term& conj,
                 const std::vector<Term>& grammarAssertions) const;

  /**
   * Get an abduct using a subset of assertions for grammar generation.
   *
   * This is a variant of getAbduct() that allows specifying both a grammar
   * and which assertions should be used for grammar generation.
   *
   * @warning This function is experimental and may change in future versions.
   *
   * @param conj The conjecture term.
   * @param grammar The grammar for the abduct.
   * @param grammarAssertions The subset of current assertions to use for
   *        grammar generation.
   * @return The abduct term, or null if none found.
   */
