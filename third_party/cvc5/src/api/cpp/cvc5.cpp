...
Term Solver::getAbduct(const Term& conj,
                       const std::vector<Term>& grammarAssertions) const
{
  ...
  CVC5_API_CHECK(d_slv->getOptions().smt.produceAbducts)
      ...
      << internal::options::smt::longName::produceAbducts << ")";
  //////// all checks before this line
  std::vector<internal::Node> grammarAxioms;
  for (const Term& t : grammarAssertions)
  {
    grammarAxioms.push_back(*t.d_node);
  }
  internal::TypeNode nullType;
  internal::Node result =
      d_slv->getAbduct(*conj.d_node, nullType, grammarAxioms);
  return Term(&d_tm, result);
  ////////
  CVC5_API_TRY_CATCH_END;
}
                       ...
                       const std::vector<Term>& grammarAssertions) const
{
  ...
  internal::Node result =
      d_slv->getAbduct(*conj.d_node, *grammar.resolve().d_type, grammarAxioms);
  return Term(&d_tm, result);
  ////////
  CVC5_API_TRY_CATCH_END;
}
