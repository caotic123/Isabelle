...
bool AbductionSolver::getAbduct(const std::vector<Node>& axioms,
                                ...
                                Node& abd)
{
  // Pass empty grammarAxioms to use old behavior (grammar from all asserts)
  std::vector<Node> emptyGrammarAxioms;
  return getAbduct(axioms, emptyGrammarAxioms, goal, grammarType, abd);
}

bool AbductionSolver::getAbduct(const std::vector<Node>& axioms,
                                const std::vector<Node>& grammarAxioms,
                                const Node& goal,
                                const TypeNode& grammarType,
                                Node& abd)
{
  ...
  Trace("sygus-abduct") << "Grammar axioms: " << grammarAxioms << std::endl;
  ...
  // normalize grammar axioms separately
  std::vector<Node> grammarAxiomsn;
  for (const Node& ax : grammarAxioms)
  {
    grammarAxiomsn.emplace_back(tls.apply(ax));
  }
  ...
  // grammarAxiomsn controls what symbols go into the grammar.
  // If empty, mkAbductionConjecture falls back to using asserts (old behavior).
  // The caller is responsible for including the goal in grammarAxiomsn if needed.
  // Note: axiomsn is used for the side condition, grammarAxiomsn for symbol extraction.
  Node aconj = quantifiers::SygusAbduct::mkAbductionConjecture(
      nodeManager(), name, asserts, axiomsn, grammarAxiomsn, grammarType);
