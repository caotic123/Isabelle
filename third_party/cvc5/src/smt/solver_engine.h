...
class CVC5_EXPORT SolverEngine
{
 ...
 public:
   ...
   * This method asks this SMT engine to find an abduct with respect to the
   * current assertion stack (call it A) and the conjecture (call it B),
   * using only a subset of assertions for grammar generation.
   *
   * @param conj The conjecture B.
   * @param grammarType A sygus datatype type that encodes the syntax
   * restrictions on the shape of possible solutions.
   * @param grammarAxioms A subset of the current assertions to use for
   * grammar generation (symbol extraction). All assertions are still used
   * for the logical constraint.
   * @return The abduct C, or null if none exists.
   */
  Node getAbduct(const Node& conj,
