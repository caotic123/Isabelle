...
class AbductionSolver : protected EnvObj
{
 public:
  ...
  /**
   * This method asks this SMT engine to find an abduct with respect to the
   * current assertion stack (call it A) and the goal (call it B), using only
   * a subset of assertions for grammar generation.
   *
   * @param axioms The expanded assertions A of the parent SMT engine
   * @param grammarAxioms A subset of axioms to use for grammar generation
   * (symbol extraction and constant collection). All axioms are still used
