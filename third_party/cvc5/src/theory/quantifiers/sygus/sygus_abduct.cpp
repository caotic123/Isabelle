...
Node SygusAbduct::mkAbductionConjecture(NodeManager* nm,
                                        ...
                                        TypeNode abdGType)
{
  // Delegate to overload with asserts as grammarSources (default behavior)
  return mkAbductionConjecture(nm, name, asserts, axioms, asserts, abdGType);
}
...
Node SygusAbduct::mkAbductionConjecture(NodeManager* nm,
                                        ...
                                        const std::vector<Node>& asserts,
                                        const std::vector<Node>& axioms,
                                        const std::vector<Node>& grammarSources,
                                        TypeNode abdGType)
{
  // Use grammarSources for symbol extraction (controls what's in the grammar)
  const std::vector<Node>& symbolSources =
      grammarSources.empty() ? asserts : grammarSources;
  std::unordered_set<Node> symset;
  for (size_t i = 0, size = symbolSources.size(); i < size; i++)
  {
    expr::getSymbols(symbolSources[i], symset);
  }
