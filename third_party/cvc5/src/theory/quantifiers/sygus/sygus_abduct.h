...
class SygusAbduct
{
 public:
  ...
  /**
   * Overload that allows specifying a separate set of assertions for grammar
   * symbol extraction.
   *
   * @param grammarSources Assertions to use for extracting symbols for the
   *        grammar. If empty, uses asserts for symbol extraction (default
   *        behavior).
   */
  static Node mkAbductionConjecture(NodeManager* nm,
                                    const std::string& name,
