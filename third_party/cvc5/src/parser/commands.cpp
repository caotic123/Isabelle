...
GetAbductCommand::GetAbductCommand(const std::string& name, cvc5::Term conj)
    : d_name(name),
      ...
      d_useGrammarAssertions(false)
{
}
GetAbductCommand::GetAbductCommand(const std::string& name,
                                   cvc5::Term conj,
                                   cvc5::Grammar* g)
    : d_name(name), d_conj(conj), d_sygus_grammar(g), d_useGrammarAssertions(false)
{
}
GetAbductCommand::GetAbductCommand(
    ...
    cvc5::Term conj,
    cvc5::Grammar* g,
    const std::vector<std::string>& grammarAssertionNames)
    : d_name(name),
      d_conj(conj),
      d_sygus_grammar(g),
      d_grammarAssertionNames(grammarAssertionNames),
      d_useGrammarAssertions(true)
{
}
...
void GetAbductCommand::invoke(cvc5::Solver* solver, SymManager* sm)
{
  try
  {
    ...
    // Check if :grammar-assertions was specified (even if empty)
    if (d_useGrammarAssertions)
    {
      // Resolve named assertions to terms
      std::map<cvc5::Term, std::string> namedTerms = sm->getExpressionNames();
      // Always include the goal's symbols in the grammar
      std::vector<cvc5::Term> grammarAssertions;
      grammarAssertions.push_back(d_conj);
      // Add the specified named assertions
      for (const std::string& name : d_grammarAssertionNames)
      {
        auto it = std::find_if(
            namedTerms.begin(),
            namedTerms.end(),
            [&name](const auto& pair) { return pair.second == name; });
        if (it == namedTerms.end())
        {
          throw cvc5::CVC5ApiException("Unknown named assertion: " + name);
        }
        grammarAssertions.push_back(it->first);
      }

      if (d_sygus_grammar == nullptr)
      {
        d_result = solver->getAbduct(d_conj, grammarAssertions);
      }
      else
      {
        d_result =
            solver->getAbduct(d_conj, *d_sygus_grammar, grammarAssertions);
      }
    }
    else
    {
      // Original behavior: use all assertions for grammar generation
      if (d_sygus_grammar == nullptr)
      {
