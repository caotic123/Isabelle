...
std::unique_ptr<Cmd> Smt2CmdParser::parseNextCommand()
{
  ...
  switch (tok)
  {
    ...
    break;
    // (get-abduct <symbol> <term> <grammar>? :grammar-assertions?)
    case Token::GET_ABDUCT_TOK:
    {
      ...
      // parse optional :grammar-assertions keyword
      // Note: parseGrammarOrNull may have peeked, so use nextToken not peekToken
      std::vector<std::string> grammarAssertionNames;
      bool useGrammarAssertions = false;
      tok = d_lex.nextToken();
      if (tok == Token::KEYWORD)
      {
        // get keyword string and strip the colon prefix
        std::string key = d_lex.tokenStr();
        key.erase(0, 1);
        if (key == "grammar-assertions")
        {
          useGrammarAssertions = true;
          // parse list of assertion names: (name1 name2 ...)
          d_lex.eatToken(Token::LPAREN_TOK);
          tok = d_lex.peekToken();
          while (tok != Token::RPAREN_TOK)
          {
            ...
            grammarAssertionNames.push_back(assertName);
            tok = d_lex.peekToken();
          }
          d_lex.eatToken(Token::RPAREN_TOK);
        }
        ...
        {
          d_lex.parseError("Unexpected keyword in get-abduct");
        }
      }
      else
      {
        // Not a keyword, put the token back for final RPAREN consumption
        d_lex.reinsertToken(tok);
      }
      // Use the appropriate constructor based on whether :grammar-assertions was specified
      if (useGrammarAssertions)
      {
        cmd.reset(new GetAbductCommand(name, t, g, grammarAssertionNames));
      }
      else if (g != nullptr)
      {
      ...
      {
        cmd.reset(new GetAbductCommand(name, t));
      }
    }
    break;
    // (get-abduct-next)
